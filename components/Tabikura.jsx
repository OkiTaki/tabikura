...
function Avt({i,c,s=32}){
  return(
    <div style={{width:s,height:s,borderRadius:"50%",background:c,display:"flex",alignItems:"center",
      justifyContent:"center",color:"white",fontWeight:800,fontSize:s*0.38,flexShrink:0}}>
      {i}
    </div>
  );
}

// me をpropsで受け取る（グローバル変数を使わない）
function ReactionBar({reactions,onReact,me}){
    ...
    <div style={{padding:"0 10px 6px",display:"flex",gap:4,flexWrap:"wrap",alignItems:"center"}}>
      {Object.entries(reactions).map(([emoji,users])=>{
        ...
        return(
          <button key={emoji} onClick={()=>onReact(emoji)}
            style={{background:isMine?"#6C63FF18":"#F5F0E8",border:`1px solid ${isMine?"#6C63FF55":"transparent"}`,
              borderRadius:20,padding:"4px 9px",cursor:"pointer",fontSize:13,
              color:isMine?"#6C63FF":"#555",fontWeight:isMine?700:400,
      ...
      <div style={{position:"relative"}} ref={ref}>
        <button onClick={()=>setShowPicker(p=>!p)}
          style={{background:showPicker?"#1A1A2E":"#F0EEF8",border:"none",borderRadius:20,
            padding:"4px 10px",cursor:"pointer",color:showPicker?"white":"#888",fontSize:14,fontWeight:700,lineHeight:1}}>
          ＋
        </button>
        {showPicker&&(
            ...
            <div style={{display:"flex",flexWrap:"wrap",gap:4,marginBottom:10}}>
              {EMOJI_CANDIDATES.map(e=>(
                <button key={e} onClick={()=>{onReact(e);setShowPicker(false);}}
                  style={{background:"#F7F3EE",border:"none",borderRadius:8,padding:"5px 7px",cursor:"pointer",fontSize:17}}>
                  {e}
                </button>
              ))}
            </div>
            <div style={{borderTop:"1px solid #F0EEF8",paddingTop:9}}>
              ...
              <div style={{display:"flex",gap:5}}>
                <input value={customEmoji} onChange={e=>setCustomEmoji(e.target.value)}
                  ...
                  style={{flex:1,border:"1.5px solid #E8E0D5",borderRadius:8,padding:"6px 8px",fontSize:18,outline:"none",textAlign:"center"}}/>
                <button onClick={()=>{if(customEmoji.trim()){onReact(customEmoji.trim());setCustomEmoji("");setShowPicker(false);}}}
                  style={{background:"#6C63FF",color:"white",border:"none",borderRadius:8,padding:"6px 12px",cursor:"pointer",fontSize:11,fontWeight:700}}>
                  追加
                </button>
              </div>
            </div>
...
function PostCard({post,gps,expanded,onExpand,onToggleDone,onReact,onAddComment,
  onEditComment,onDeleteComment,onEdit,onDelete,isItinerary,me,myAvt}){
  ...
  if(editing) return(
    <div style={{background:"white",borderRadius:14,marginBottom:10,padding:14,
      boxShadow:"0 1px 8px rgba(0,0,0,0.05)",border:"2px solid #6C63FF44"}}>
      <div style={{fontSize:11,color:"#6C63FF",fontWeight:700,marginBottom:10}}>✏️ 編集中</div>
      <input value={editTitle} onChange={e=>setEditTitle(e.target.value)}
        onKeyDown={e=>e.key==="Enter"&&!e.nativeEvent.isComposing&&submitEdit()} autoFocus
        placeholder="場所・お店名"
        style={{width:"100%",border:"1.5px solid #E8E0D5",borderRadius:10,padding:"8px 12px",fontSize:13,outline:"none",marginBottom:10}}/>
      <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:10}}>
        {CATEGORIES.filter(c=>c!=="すべて").map(cat=>(
          <button key={cat} onClick={()=>setEditCat(cat)}
            style={{background:editCat===cat?"#1A1A2E":"#F5F0E8",color:editCat===cat?"white":"#888",
              border:"none",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:10,fontWeight:editCat===cat?700:400}}>
            {CAT_EMOJI[cat]} {cat}
          </button>
        ))}
      </div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:12}}>
        <div style={{flex:1}}>
          ...
          <input value={editHours} onChange={e=>setEditHours(e.target.value)} placeholder="例: 10:00〜22:00"
            style={{width:"100%",border:"1.5px solid #E8E0D5",borderRadius:10,padding:"7px 10px",fontSize:12,outline:"none"}}/>
        </div>
        <div style={{flex:1}}>
          <div style={{fontSize:10,color:"#AAA",fontWeight:600,marginBottom:4}}>🚫 定休日</div>
          <input value={editClosed} onChange={e=>setEditClosed(e.target.value)} placeholder="例: 水曜定休"
            style={{width:"100%",border:"1.5px solid #E8E0D5",borderRadius:10,padding:"7px 10px",fontSize:12,outline:"none"}}/>
        </div>
      </div>
      <div style={{display:"flex",gap:8}}>
        <button onClick={cancelEdit}
          style={{flex:1,background:"#F5F0E8",color:"#888",border:"none",borderRadius:10,padding:"8px",cursor:"pointer",fontWeight:600,fontSize:12}}>
          キャンセル
        </button>
        <button onClick={submitEdit}
          style={{flex:2,background:"#6C63FF",color:"white",border:"none",borderRadius:10,padding:"8px",cursor:"pointer",fontWeight:700,fontSize:12}}>
          保存 ✓
        </button>
      </div>
    </div>
  ...
  return(
    <div style={{background:"white",borderRadius:14,marginBottom:10,overflow:"visible",
      boxShadow:"0 1px 8px rgba(0,0,0,0.05)",
      border:post.done?"2px solid #2A9D8F30":"2px solid transparent",opacity:post.done?0.8:1}}>
      <div style={{padding:"14px 14px 6px",display:"flex",gap:10}}>
        ...
        <div style={{flex:1,minWidth:0}}>
          <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:5}}>
            ...
            <div style={{marginLeft:"auto",position:"relative"}}>
              <button onClick={()=>setShowMenu(p=>!p)}
                style={{background:"none",border:"none",cursor:"pointer",color:"#BBB",fontSize:18,padding:"0 4px",lineHeight:1}}>
                ⋯
              </button>
              {showMenu&&(
                <div style={{position:"absolute",right:0,top:"100%",background:"white",borderRadius:10,
                  boxShadow:"0 4px 20px rgba(0,0,0,0.15)",zIndex:100,overflow:"hidden",minWidth:110}}>
                  <button onClick={()=>{setEditing(true);setShowMenu(false);}}
                    style={{width:"100%",background:"none",border:"none",padding:"10px 14px",cursor:"pointer",
                      fontSize:12,color:"#333",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
                    ✏️ 編集
                  </button>
                  <button onClick={()=>{setShowMenu(false);onDelete(post.id);}}
                    style={{width:"100%",background:"none",border:"none",padding:"10px 14px",cursor:"pointer",
                      fontSize:12,color:"#E05",textAlign:"left",display:"flex",alignItems:"center",gap:8}}>
                    🗑 削除
                  </button>
                </div>
              )}
          ...
          <div style={{display:"flex",alignItems:"flex-start",gap:8}}>
            <button onClick={()=>onToggleDone(post.id)}
              style={{background:"none",border:"none",cursor:"pointer",padding:0,marginTop:2,flexShrink:0}}>
              <div style={{width:20,height:20,borderRadius:5,
                border:post.done?"none":"2px solid #DDD",
                background:post.done?"#2A9D8F":"white",
            ...
            <div style={{minWidth:0}}>
              <div style={{fontWeight:700,fontSize:14,
                color:post.done?"#AAA":"#1A1A2E",
                textDecoration:post.done?"line-through":"none"}}>
                {post.title}
              </div>
              <div style={{display:"flex",gap:6,marginTop:4,flexWrap:"wrap",alignItems:"center"}}>
                <span style={{background:"#1A1A2E10",color:"#1A1A2E",borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700}}>
                  {CAT_EMOJI[post.category]||"📍"} {post.category}
                </span>
                {isItinerary&&post.planDate&&(
                  <span style={{background:"#6C63FF12",color:"#6C63FF",borderRadius:20,padding:"2px 9px",
      ...
      <ReactionBar reactions={post.reactions||{}} onReact={emoji=>onReact(post.id,emoji)} me={me}/>

      <div style={{padding:"0 10px 10px",display:"flex",gap:4,alignItems:"center"}}>
        <button onClick={()=>onExpand(post.id)}
          style={{background:expanded?"#6C63FF18":"#F5F0E8",
            border:`1px solid ${expanded?"#6C63FF55":"transparent"}`,
            borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:11,
            color:expanded?"#6C63FF":"#AAA",fontWeight:expanded?700:400,
            display:"flex",alignItems:"center",gap:3}}>
          💬{post.comments.length>0&&` ${post.comments.length}`}
        </button>
        <a href={mapsUrl(post.title,post.location)} target="_blank" rel="noopener noreferrer"
          style={{background:"#E8F4FE",border:"1px solid #4285F422",borderRadius:20,padding:"4px 10px",
            fontSize:10,color:"#4285F4",fontWeight:700,textDecoration:"none",display:"flex",alignItems:"center",gap:4}}>
      ...
      {expanded&&(
        <div style={{borderTop:"1px solid #F5F0E8",background:"#FAFAF8"}}>
                ...
                {editingComment===c.id?(
                  <div style={{background:"white",borderRadius:12,padding:"8px 12px",border:"1.5px solid #6C63FF44"}}>
                    <input value={editCommentText} onChange={e=>setEditCommentText(e.target.value)}
                      onKeyDown={e=>{
                        if(e.key==="Enter"&&!e.nativeEvent.isComposing) submitEditComment(c.id);
                    ...
                    <div style={{display:"flex",gap:6,marginTop:6,justifyContent:"flex-end"}}>
                      ...
                      <button onClick={()=>submitEditComment(c.id)}
                        style={{background:"#6C63FF",color:"white",border:"none",borderRadius:8,padding:"3px 10px",cursor:"pointer",fontSize:11,fontWeight:700}}>
                        保存
                      </button>
                    </div>
                  </div>
                      ...
                      {c.author===me&&(
                        <div style={{marginLeft:"auto",display:"flex",gap:2}}>
                          <button onClick={()=>startEditComment(c)}
                            style={{background:"none",border:"none",cursor:"pointer",fontSize:11,color:"#CCC",padding:"0 3px"}}>
                            ✏️
                          </button>
          ...
          <div style={{padding:"8px 12px 12px",display:"flex",gap:6}}>
            <Avt i={myAvt} c={MY_CLR} s={26}/>
            <input value={commentText} onChange={e=>setCommentText(e.target.value)}
              onKeyDown={e=>e.key==="Enter"&&!e.nativeEvent.isComposing&&submitComment()}
              placeholder="コメントを追加..."
              style={{flex:1,border:"1.5px solid #E8E0D5",borderRadius:20,padding:"6px 14px",fontSize:12,outline:"none"}}/>
            <button onClick={submitComment}
              style={{background:"#6C63FF",color:"white",border:"none",borderRadius:20,padding:"6px 14px",fontSize:11,cursor:"pointer",fontWeight:700}}>
              送信
            </button>
          </div>
        </div>
...
export default function App(){
  ...
  useEffect(()=>{
    ...
    const loadData=async()=>{
      // localStorageのニックネームをusersテーブルで検証
      ...
      const chList=(chs||[])
        .filter(c=>(c.members||[]).includes(validNick))
        .map(c=>({id:c.id,name:c.name,color:c.color,type:c.type,members:c.members||[]}));

      setChannels(chList);
      setActiveChannel(prev=>{
        if(prev&&chList.find(c=>c.id===prev)) return prev;
        return chList.length?chList[0].id:null;
      });
      setOnboarding(false);
  ...
  // --- ここから下では必ず loaded===true かつ nickname が確定している ---
  const ME=nickname;
  const MY_AVT=ME&&ME.length>0?ME[0].toUpperCase():"?";
  ...
  const editPost=async(id,title,category,planDate,planTime,hours,closed)=>{
    setPosts(p=>({...p,[activeChannel]:p[activeChannel].map(x=>x.id!==id?x:
      {...x,title,category,planDate:planDate||null,planTime:planTime||null,hours:hours||null,closed:closed||null})}));
    await supabase.from("posts").update({title,category,
      plan_date:planDate||null,plan_time:planTime||null,hours:hours||null,closed:closed||null}).eq("id",id);
  };
  ...
  const addPost=async()=>{
    ...
    const id="p"+Date.now();
    const post={id,author:ME,avatar:MY_AVT,avatarColor:MY_CLR,
  ...
  const openChSettings=(c)=>{
    setEditCh({name:c.name,type:c.type||"memo",color:c.color});
    setShowDeleteConfirm(false);
    setShowChSettings(c.id);
  };
  ...
  const removeMember=async(name)=>{
    const tch=channels.find(c=>c.id===activeChannel);
    if(!tch) return;
    const newMembers=tch.members.filter(x=>x!==name);
  ...
  const getInviteUrl=(chId)=>{
    const tch=channels.find(c=>c.id===chId);
    if(!tch) return "";
    const data={id:tch.id,name:tch.name,color:tch.color,type:tch.type};
  ...
  if(onboarding) return(
    <div style={{height:"100vh",background:"#18172B",display:"flex",flexDirection:"column",
      alignItems:"center",justifyContent:"center",fontFamily:"'Hiragino Sans','Noto Sans JP',sans-serif",padding:24}}>
      <style>{`*{box-sizing:border-box;margin:0;padding:0;}button,input{font-family:inherit;}`}</style>
      <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:40}}>
        <div style={{width:48,height:48,borderRadius:16,background:"linear-gradient(135deg,#6C63FF,#F4A261)",
      ...
      <div style={{background:"#22203A",borderRadius:24,padding:"28px 24px",width:"100%",maxWidth:360,
        marginBottom:28,minHeight:160,display:"flex",flexDirection:"column",alignItems:"center",
      ...
      <div style={{display:"flex",gap:6,marginBottom:28}}>
        {STEPS.map((_,i)=>(
          <div key={i} style={{width:i===onboardingStep?20:6,height:6,borderRadius:3,
            background:i===onboardingStep?"#6C63FF":"#2A2940",transition:"width 0.2s"}}/>
        ))}
      </div>
      {onboardingStep<STEPS.length-1?(
        <div style={{display:"flex",gap:10,width:"100%",maxWidth:360}}>
          <button onClick={()=>setOnboardingStep(STEPS.length-1)}
            style={{flex:1,background:"transparent",border:"1px solid #2A2940",borderRadius:14,
              padding:"13px",color:"#6660A0",cursor:"pointer",fontWeight:600,fontSize:13}}>
            スキップ
          </button>
          <button onClick={()=>setOnboardingStep(s=>s+1)}
            style={{flex:2,background:"#6C63FF",border:"none",borderRadius:14,padding:"13px",
              color:"white",cursor:"pointer",fontWeight:700,fontSize:14,boxShadow:"0 4px 20px #6C63FF44"}}>
            次へ →
          </button>
        </div>
      ):(
  ...
  // ── メイン画面 ──
  return(
    <div style={{display:"flex",height:"100vh",background:"#F7F3EE",
      ...
      <div className={`sidebar${showSidebar?" open":""}`} style={{display:showSidebar?"flex":"none",flexDirection:"column",height:"100%"}}>
        <div style={{padding:"16px 14px",flex:1,overflowY:"auto",minHeight:0}}>
          ...
          <div style={{background:"#22203A",borderRadius:8,padding:"5px 10px",marginBottom:12,display:"flex",alignItems:"center",gap:6}}>
            ...
            <div style={{color:gps?"#2A9D8F":"#555",fontSize:10,fontWeight:600}}>
              {gps?"現在地 取得済み":"現在地 取得中..."}
            </div>
          </div>
          ...
          {channels.map(c=>(
            <div key={c.id} style={{position:"relative",marginBottom:2}}>
              <button onClick={()=>{setActiveChannel(c.id);setShowSidebar(false);}}
                style={{width:"100%",background:activeChannel===c.id?"#2A2940":"transparent",
                  border:"none",borderRadius:8,padding:"7px 10px",cursor:"pointer",
                  display:"flex",alignItems:"center",gap:8,textAlign:"left",paddingRight:28}}>
                <div style={{width:6,height:6,borderRadius:"50%",background:c.color,flexShrink:0}}/>
                <div style={{flex:1,minWidth:0}}>
                  <div style={{color:activeChannel===c.id?"white":"#8884AA",fontSize:11,fontWeight:600,
                ...
                <div style={{background:"#2A2940",color:"#6660A0",fontSize:10,borderRadius:10,padding:"1px 5px",flexShrink:0}}>
                  {(posts[c.id]||[]).length}
                </div>
              </button>
              <button onClick={e=>{e.stopPropagation();openChSettings(c);}}
                style={{position:"absolute",right:4,top:"50%",transform:"translateY(-50%)",
                  background:"none",border:"none",cursor:"pointer",color:"#4A4870",
                  fontSize:12,padding:"3px 4px",lineHeight:1,borderRadius:6,opacity:0.7}}>
                ⚙️
              </button>
            </div>
          ))}
          {showNewCh?(
            <div style={{background:"#22203A",borderRadius:10,padding:10,marginTop:4}}>
              <input value={newCh.name} onChange={e=>setNewCh(p=>({...p,name:e.target.value}))}
                onKeyDown={e=>e.key==="Enter"&&!e.nativeEvent.isComposing&&addChannel()}
                placeholder="チャネル名..." autoFocus
                style={{width:"100%",background:"#18172B",border:"none",borderRadius:6,
                  padding:"6px 8px",color:"white",fontSize:11,outline:"none",marginBottom:8}}/>
              <div style={{display:"flex",gap:5,marginBottom:8}}>
                {Object.entries(CH_TYPES).map(([key,val])=>(
                  <button key={key} onClick={()=>setNewCh(p=>({...p,type:key}))}
                    style={{flex:1,background:newCh.type===key?"#6C63FF":"#18172B",
                      color:newCh.type===key?"white":"#6660A0",
                      border:`1px solid ${newCh.type===key?"#6C63FF":"#2A2940"}`,
                      borderRadius:8,padding:"6px 4px",cursor:"pointer",fontSize:10,fontWeight:700,textAlign:"center"}}>
                    {val.icon}
                    <div style={{fontSize:9,marginTop:2}}>{val.label}</div>
                  </button>
                ))}
              </div>
              <div style={{display:"flex",gap:4}}>
                <button onClick={addChannel}
                  style={{flex:1,background:"#6C63FF",color:"white",border:"none",borderRadius:6,
                    padding:"5px 0",fontSize:11,cursor:"pointer",fontWeight:700}}>
                  作成
                </button>
                <button onClick={()=>setShowNewCh(false)}
                  style={{background:"#333",color:"#888",border:"none",borderRadius:6,padding:"5px 8px",fontSize:11,cursor:"pointer"}}>
                  ×
                </button>
              </div>
            </div>
            ...
            <button onClick={()=>setShowNewCh(true)}
              style={{width:"100%",background:"transparent",border:"1px dashed #2A2940",
                borderRadius:8,padding:"6px",color:"#4A4870",cursor:"pointer",fontSize:11,marginTop:4}}>
              ＋ 新しいチャネル
            </button>
          )}
        </div>
        ...
        <div style={{padding:"10px 14px",borderTop:"1px solid #22203A",display:"flex",alignItems:"center",gap:8,flexShrink:0}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{color:"white",fontSize:11,fontWeight:700,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
              {ME}
            </div>
            <div style={{color:"#4A4870",fontSize:9}}>オーナー</div>
          </div>
          <button onClick={()=>{
            const n=prompt("ニックネームを変更",ME);
            if(n?.trim()){setNickname(n.trim());localStorage.setItem("tabikura_nickname",n.trim());}
          }} style={{background:"none",border:"none",cursor:"pointer",color:"#4A4870",fontSize:11,padding:"3px 5px",borderRadius:6}}>
            ✏️
          </button>
        </div>
      </div>
        ...
        <div style={{background:"white",borderBottom:"1px solid #EDE8E0",padding:"10px 12px",
          ...
          <div style={{flex:1,minWidth:0}}>
            <div style={{display:"flex",alignItems:"center",gap:6,flexWrap:"wrap"}}>
              <div style={{fontWeight:900,fontSize:15,color:"#1A1A2E",whiteSpace:"nowrap",
                overflow:"hidden",textOverflow:"ellipsis"}}>
                {ch?.name}
              </div>
              <span style={{background:isItinerary?"#6C63FF12":"#F4A26120",
                color:isItinerary?"#6C63FF":"#F4A261",borderRadius:20,padding:"2px 8px",
                fontSize:10,fontWeight:700,flexShrink:0}} className="header-badge">
                {ch&&CH_TYPES[ch.type||"memo"].icon} {ch&&CH_TYPES[ch.type||"memo"].label}
              </span>
            </div>
            <div style={{fontSize:10,color:"#AAA",marginTop:1,whiteSpace:"nowrap",
              overflow:"hidden",textOverflow:"ellipsis"}} className="header-members">
              {ch?.members.join(" · ")} · {doneCount}/{allPosts.length} 完了
            </div>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:4,flexShrink:0}} className="progress-bar">
            <div style={{fontSize:11,color:"#2A9D8F",fontWeight:700}}>
              {allPosts.length?Math.round(doneCount/allPosts.length*100):0}%
            </div>
            <div style={{width:40,height:4,background:"#EEE",borderRadius:3,overflow:"hidden"}}>
              <div style={{width:`${allPosts.length?doneCount/allPosts.length*100:0}%`,
                height:"100%",background:"#2A9D8F",borderRadius:3}}/>
            </div>
          </div>
          <button onClick={()=>setShowInvite(true)}
            style={{background:"#F5F0E8",color:"#555",border:"none",borderRadius:10,
              padding:"7px 10px",cursor:"pointer",fontWeight:700,fontSize:11,flexShrink:0}}>
            👤＋
          </button>
          <button onClick={()=>setShowNewPost(true)}
            style={{background:"#6C63FF",color:"white",border:"none",borderRadius:10,
              padding:"7px 12px",cursor:"pointer",fontWeight:700,fontSize:12,
              boxShadow:"0 4px 12px #6C63FF44",flexShrink:0}}>
            ＋ 追加
          </button>
        </div>

        {/* フィルターバー */}
        <div style={{background:"white",borderBottom:"1px solid #EDE8E0",padding:"6px 16px",
          display:"flex",gap:4,overflowX:"auto",alignItems:"center"}}>
          {CATEGORIES.map(cat=>(
            <button key={cat} onClick={()=>setFilterCat(cat)}
              style={{background:filterCat===cat?"#1A1A2E":"#F5F0E8",
                color:filterCat===cat?"white":"#888",border:"none",borderRadius:20,
                padding:"4px 10px",cursor:"pointer",fontSize:10,
                fontWeight:filterCat===cat?700:400,whiteSpace:"nowrap"}}>
              {cat!=="すべて"&&CAT_EMOJI[cat]+" "}{cat}
            </button>
          ))}
          ...
          {[["すべて","#888"],["未完了","#F4A261"],["完了","#2A9D8F"]].map(([f,col])=>(
            <button key={f} onClick={()=>setFilterDone(f)}
              style={{background:filterDone===f?col:"#F5F0E8",color:filterDone===f?"white":"#888",
                border:"none",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:10,
                fontWeight:filterDone===f?700:400,whiteSpace:"nowrap"}}>
              {f==="完了"?"✓ ":""}{f}
            </button>
          ))}
          {isItinerary&&(
            <>
              <div style={{width:1,height:16,background:"#EDE8E0",flexShrink:0,margin:"0 3px"}}/>
              <button onClick={()=>setSortByDate(p=>!p)}
                style={{background:sortByDate?"#1A1A2E":"#F5F0E8",color:sortByDate?"white":"#888",
                  border:"none",borderRadius:20,padding:"4px 10px",cursor:"pointer",fontSize:10,
                  fontWeight:sortByDate?700:400,whiteSpace:"nowrap",display:"flex",alignItems:"center",gap:3}}>
                🗓 {sortByDate?"日時順":"追加順"}
              </button>
            </>
          )}
        ...
        <div style={{flex:1,overflowY:"auto",padding:"12px 16px"}}>
          {!activeChannel?(
            <div style={{textAlign:"center",padding:"80px 20px",color:"#CCC"}}>
              ...
              <button onClick={()=>{setShowSidebar(true);setShowNewCh(true);}}
                style={{background:"#6C63FF",color:"white",border:"none",borderRadius:14,
                  padding:"12px 24px",cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 16px #6C63FF44"}}>
                ＋ 最初のチャネルを作る
              </button>
            </div>
          ):filtered.length===0?(
            ...
            Object.entries(grouped).map(([dateKey,groupPosts])=>(
              <div key={dateKey}>
                {dateKey!=="__nodate__"&&<DateGroupHeader dateStr={dateKey} posts={groupPosts}/>}
      ...
      {showNewPost&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",
          alignItems:"flex-end",justifyContent:"center",zIndex:1000}} onClick={()=>setShowNewPost(false)}>
          <div style={{background:"white",borderRadius:"24px 24px 0 0",width:"100%",maxWidth:600,
            padding:"24px 20px 32px",boxShadow:"0 -8px 40px rgba(0,0,0,0.15)"}} onClick={e=>e.stopPropagation()}>
            <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
              <div style={{width:36,height:36,borderRadius:12,background:"linear-gradient(135deg,#6C63FF,#F4A261)",
                display:"flex",alignItems:"center",justifyContent:"center",fontSize:18}}>＋</div>
              <div style={{fontWeight:900,fontSize:16,color:"#1A1A2E"}}>新しいスポットを追加</div>
              <button onClick={()=>setShowNewPost(false)}
                style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",color:"#CCC",fontSize:22,lineHeight:1}}>
                ×
              </button>
            </div>
            <input value={newPost.title} onChange={e=>setNewPost(p=>({...p,title:e.target.value}))}
              onKeyDown={e=>e.key==="Enter"&&!e.nativeEvent.isComposing&&addPost()}
              placeholder="場所・お店名を入力..." autoFocus
              style={{width:"100%",border:"1.5px solid #E8E0D5",borderRadius:12,
                padding:"12px 14px",fontSize:14,outline:"none",marginBottom:14}}/>
            <div style={{display:"flex",gap:4,flexWrap:"wrap",marginBottom:14}}>
              {CATEGORIES.filter(c=>c!=="すべて").map(cat=>(
                <button key={cat} onClick={()=>setNewPost(p=>({...p,category:cat}))}
                  style={{background:newPost.category===cat?"#1A1A2E":"#F5F0E8",
                    color:newPost.category===cat?"white":"#888",border:"none",borderRadius:20,
                    padding:"5px 12px",cursor:"pointer",fontSize:11,fontWeight:newPost.category===cat?700:400}}>
                  {CAT_EMOJI[cat]} {cat}
                </button>
              ))}
            </div>
            {isItinerary&&(
              <div style={{display:"flex",gap:8,marginBottom:14}}>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:"#AAA",fontWeight:600,marginBottom:5}}>📅 日付（任意）</div>
                  <input type="date" value={newPost.planDate} onChange={e=>setNewPost(p=>({...p,planDate:e.target.value}))}
                    style={{width:"100%",border:"1.5px solid #E8E0D5",borderRadius:10,padding:"8px 10px",fontSize:12,outline:"none"}}/>
                </div>
                <div style={{flex:1}}>
                  <div style={{fontSize:10,color:"#AAA",fontWeight:600,marginBottom:5}}>🕐 時刻（任意）</div>
            ...
            <div style={{display:"flex",gap:8}}>
              <button onClick={()=>setShowNewPost(false)}
                style={{flex:1,background:"#F5F0E8",color:"#888",border:"none",borderRadius:10,padding:"10px",cursor:"pointer",fontWeight:600}}>
                キャンセル
              </button>
              <button onClick={addPost}
                style={{flex:2,background:"#6C63FF",color:"white",border:"none",borderRadius:10,
                  padding:"10px",cursor:"pointer",fontWeight:700,fontSize:12,boxShadow:"0 4px 12px #6C63FF44"}}>
                追加する
              </button>
            </div>
          </div>
      ...
      {showInvite&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",
          alignItems:"center",justifyContent:"center",zIndex:1000,padding:20}} onClick={()=>setShowInvite(false)}>
          <div style={{background:"white",borderRadius:24,width:"100%",maxWidth:400,
            boxShadow:"0 24px 60px rgba(0,0,0,0.25)",overflow:"hidden"}} onClick={e=>e.stopPropagation()}>
            <div style={{padding:"22px 22px 0"}}>
              <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:18}}>
                ...
                <button onClick={()=>setShowInvite(false)}
                  style={{background:"none",border:"none",cursor:"pointer",color:"#CCC",fontSize:22,lineHeight:1}}>×</button>
              </div>
            </div>
            <div style={{padding:"0 22px 24px"}}>
              ...
              <div style={{background:"#F7F3EE",borderRadius:14,padding:"12px 14px",marginBottom:10}}>
                ...
                }} style={{width:"100%",background:copiedLink?"#2A9D8F":"#6C63FF",color:"white",border:"none",
                  borderRadius:10,padding:"12px",cursor:"pointer",fontWeight:700,fontSize:13,transition:"background 0.2s"}}>
                  {copiedLink?"✓ コピーしました！":"📋 リンクをコピー"}
                </button>
              </div>
              <div style={{marginTop:16}}>
                <div style={{fontSize:10,color:"#AAA",fontWeight:700,marginBottom:8,letterSpacing:1}}>
                  参加中 {ch?.members.length}人
                </div>
                <div style={{display:"flex",flexWrap:"wrap",gap:6}}>
                  {ch?.members.map((m,i)=>(
                    <div key={m} style={{display:"flex",alignItems:"center",gap:6,background:"#F7F3EE",
                      borderRadius:20,padding:"5px 10px 5px 5px"}}>
                      <div style={{width:24,height:24,borderRadius:"50%",background:MEMBER_COLORS[i%MEMBER_COLORS.length],
                        display:"flex",alignItems:"center",justifyContent:"center",color:"white",fontSize:10,fontWeight:800}}>
                        {m[0].toUpperCase()}
                      </div>
                      <div style={{fontSize:11,fontWeight:700,color:"#1A1A2E"}}>
                        {m}{m===ME&&<span style={{fontSize:9,color:"#6C63FF",marginLeft:3}}>あなた</span>}
                      </div>
                      {m!==ME&&(
                        <button onClick={()=>removeMember(m)}
                          style={{background:"none",border:"none",cursor:"pointer",color:"#CCC",fontSize:12,padding:"0 2px",lineHeight:1}}>
                          ×
                        </button>
                      )}
                    </div>
      ...
      {joinInvite&&(
        <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.6)",display:"flex",
          ...
          <div style={{background:"white",borderRadius:24,width:"100%",maxWidth:360,padding:28,
            boxShadow:"0 24px 60px rgba(0,0,0,0.3)",textAlign:"center"}}>
            <div style={{width:64,height:64,borderRadius:20,background:"linear-gradient(135deg,#6C63FF,#F4A261)",
            ...
            <div style={{background:"#F7F3EE",borderRadius:14,padding:"12px 16px",marginBottom:20,
              ...
              <span style={{background:joinInvite.type==="itinerary"?"#6C63FF12":"#F4A26120",
                color:joinInvite.type==="itinerary"?"#6C63FF":"#F4A261",
                borderRadius:20,padding:"2px 8px",fontSize:10,fontWeight:700,marginLeft:"auto"}}>
                {CH_TYPES[joinInvite.type||"memo"].icon} {CH_TYPES[joinInvite.type||"memo"].label}
              </span>
            </div>
            <div style={{display:"flex",gap:10}}>
              <button onClick={()=>setJoinInvite(null)}
                style={{flex:1,background:"#F0EEE8",border:"none",borderRadius:14,padding:"13px",
                  cursor:"pointer",fontWeight:700,fontSize:13,color:"#888"}}>
                キャンセル
              </button>
              <button onClick={acceptJoinInvite}
                style={{flex:2,background:"linear-gradient(135deg,#6C63FF,#7C73FF)",border:"none",
                  borderRadius:14,padding:"13px",cursor:"pointer",fontWeight:900,fontSize:14,
                  color:"white",boxShadow:"0 4px 20px #6C63FF44"}}>
                参加する！
              </button>
            </div>
          </div>
      ...
      {showChSettings&&(()=>{
        ...
        return(
          <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.5)",display:"flex",
            alignItems:"center",justifyContent:"center",zIndex:2000,padding:20}} onClick={()=>setShowChSettings(null)}>
            <div style={{background:"white",borderRadius:24,width:"100%",maxWidth:380,
              ...
              <div style={{padding:"20px 22px 18px",borderBottom:"1px solid #F0EEE8",display:"flex",alignItems:"center",gap:10}}>
                <div style={{width:36,height:36,borderRadius:12,background:editCh.color,
                ...
                <button onClick={()=>setShowChSettings(null)}
                  style={{marginLeft:"auto",background:"none",border:"none",cursor:"pointer",color:"#CCC",fontSize:22,lineHeight:1}}>
                  ×
                </button>
              </div>
              {!showDeleteConfirm?(
                <div style={{padding:"18px 22px 22px"}}>
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    <div style={{display:"flex",gap:8}}>
                      {Object.entries(CH_TYPES).map(([key,val])=>(
                        <button key={key} onClick={()=>setEditCh(p=>({...p,type:key}))}
                          style={{flex:1,background:editCh.type===key?"#1A1A2E":"#F7F3EE",
                            color:editCh.type===key?"white":"#888",
                            border:`2px solid ${editCh.type===key?"#1A1A2E":"transparent"}`,
                  ...
                  <div style={{marginBottom:16}}>
                    ...
                    <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
                      {["#E8A87C","#7CB9E8","#7CE8B0","#E87CB9","#C8E87C","#F4A261","#6C63FF","#2A9D8F","#E84040","#F4D261"].map(col=>(
                        <button key={col} onClick={()=>setEditCh(p=>({...p,color:col}))}
                          style={{width:28,height:28,borderRadius:"50%",background:col,
                            border:editCh.color===col?"3px solid #1A1A2E":"3px solid transparent",
                            cursor:"pointer",transform:editCh.color===col?"scale(1.2)":"scale(1)"}}/>
                      ))}
                    </div>
                  </div>
                  <div style={{marginBottom:20}}>
                    ...
                    <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:8}}>
                      {(targetCh?.members||[]).map(m=>(
                        <div key={m} style={{background:"#F5F0E8",borderRadius:20,padding:"4px 10px",
                          ...
                          {m!==ME&&(
                            ...
                            }} style={{background:"none",border:"none",cursor:"pointer",color:"#AAA",fontSize:13,lineHeight:1,padding:0}}>
                              ×
                            </button>
                          )}
                        </div>
                  ...
                  <div style={{display:"flex",gap:8,marginBottom:10}}>
                    <button onClick={()=>setShowChSettings(null)}
                      style={{flex:1,background:"#F5F0E8",color:"#888",border:"none",borderRadius:12,
                        padding:"11px",cursor:"pointer",fontWeight:600,fontSize:13}}>
                      キャンセル
                    </button>
                    <button onClick={saveChSettings}
                      style={{flex:2,background:"#6C63FF",color:"white",border:"none",borderRadius:12,
                        padding:"11px",cursor:"pointer",fontWeight:700,fontSize:13,boxShadow:"0 4px 12px #6C63FF33"}}>
                      保存する ✓
                    </button>
                  </div>
                  <button onClick={()=>setShowDeleteConfirm(true)}
                    style={{width:"100%",background:"none",border:"1.5px solid #FFD0D0",borderRadius:12,
                      padding:"10px",cursor:"pointer",fontWeight:700,fontSize:12,color:"#E05"}}>
                    🗑 このチャネルを削除する
                  </button>
                </div>
              ):(
                <div style={{padding:"22px",textAlign:"center"}}>
                  ...
                  <div style={{display:"flex",gap:8,marginTop:18}}>
                    <button onClick={()=>setShowDeleteConfirm(false)}
                      style={{flex:1,background:"#F5F0E8",color:"#888",border:"none",borderRadius:12,
                        padding:"12px",cursor:"pointer",fontWeight:700,fontSize:13}}>
                      やめる
                    </button>
                    <button onClick={deleteChannel}
                      style={{flex:1,background:"#E05",color:"white",border:"none",borderRadius:12,
                        padding:"12px",cursor:"pointer",fontWeight:700,fontSize:13}}>
                      削除する
                    </button>
                  </div>
